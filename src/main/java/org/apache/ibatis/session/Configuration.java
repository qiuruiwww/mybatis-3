...
public class Configuration {
  ...
  //全局性地开启或关闭所有映射器配置文件中已配置的任何缓存。mapper缓存，即二级缓存
  ...
  //用于注册Mapper接口信息，建立Mapper接口的class对象和Mapperproxyfactory对象之间的关系，其中Mapperproxyfactory对象用于创建Mapper动态代理
  protected final MapperRegistry mapperRegistry = new MapperRegistry(this);
  //用于注册mybatis的插件信息，mybatis插件实际上就是一个拦截器
  ...
  //MappedStatement对象描述select等标签或者通过@select等注解配置的SQL信息。
  ...
  // 其中key为resultMap的id，该id由Mapper命名空间和resultMap标签的id属性组成，value为解析resultMap标签后得到的resultMap对象
  protected final Map<String, ResultMap> resultMaps = new StrictMap<>("Result Maps collection");
  //用于注册Mapper中通过ParameterMap标签注册的参数映射信息。
  // key为ParameterMap的id，由Mapper命名空间和ParameterMap标签的id属性构成，value为解析ParameterMap标签后得到的ParameterMap对象
  protected final Map<String, ParameterMap> parameterMaps = new StrictMap<>("Parameter Maps collection");
  //用于注册KeyGenerator，KeyGenerator是mybatis的主键生成器，mybatis中提供了3中KeyGenerator，即jdbc3KeyGenerator（数据库主键自增）
  //noKeyGenerator(无自增主键),selectKeyGenerator(通过select语句查询自增主键，例如Oracle的sequence)
  protected final Map<String, KeyGenerator> keyGenerators = new StrictMap<>("Key Generators collection");

  //用于注册所有的Mapper xml配置文件路径
  protected final Set<String> loadedResources = new HashSet<>();
  //用于注册Mapper中通过SQL标签配置的SQL片段，key为SQL片段的id，value为mybatis封装的标识xml节点的xnodo对象
  ...
  /**
   * @Author Qiu Rui
   * @Description ParameterHandler 组件工厂方法
   * @Date 14:00 2020/7/25
   * @Param [mappedStatement, parameterObject, boundSql]
  ...
  /**
   * @Author Qiu Rui
   * @Description ResultSetHandler 组件工厂方法
   * @Date 14:00 2020/7/25
   * @Param [executor, mappedStatement, rowBounds, parameterHandler, resultHandler, boundSql]
   * @return org.apache.ibatis.executor.resultset.ResultSetHandler
   **/
  ...
  /**
   * @Author Qiu Rui
   * @Description StatementHandler 组件工厂方法
   * @Date 14:01 2020/7/25
   * @Param [executor, mappedStatement, parameterObject, rowBounds, resultHandler, boundSql]
  ...
  /**
   * @Author Qiu Rui
   * @Description Executor 组件工厂方法
   * @Date 14:01 2020/7/25
   * @Param [transaction, executorType]
